        ...
        @if (isset($fines[$key]))
            ...
            <div class="space-y-2">
                @foreach ($fines[$key] as $fine)
                    <div class="bg-white rounded-3xl shadow-sm p-4 flex items-start justify-between gap-4">
                        <div>
                            <div class="font-bold">{{ $fine->title }}</div>
                            <p class="text-sm text-slate-500 mt-0.5">{{ $fine->description }}</p>
                        </div>
                        <span class="shrink-0 bg-red-50 text-red-700 text-xs font-bold px-3 py-1.5 rounded-full whitespace-nowrap">
                            {{ $fine->amount_label }}
                        </span>
                    </div>
                @endforeach
